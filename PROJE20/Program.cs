    ...
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            foreach (ManagementObject obj in searcher.Get())
            {
                ...
                sb.AppendLine($"     Çekirdek    : {obj["NumberOfCores"]} çekirdek / {obj["NumberOfLogicalProcessors"]} mantıksal işlemci");
                sb.AppendLine($"     Max Hız     : {obj["MaxClockSpeed"]} MHz");
                sb.AppendLine($"     Mimari      : {GetArchitecture(obj["Architecture"]?.ToString())}");
                sb.AppendLine();
            }
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            foreach (ManagementObject obj in searcher.Get())
            {
                ...
                string ramStr = ramBytes > 0 ? $"{ramBytes / (1024 * 1024)} MB" : "Bilinmiyor";
        ...
        try
        {
            ...
            foreach (ManagementObject obj in searcher.Get())
            {
                ...
                sb.AppendLine($"     Tür         : {obj["MemoryType"] ?? "Bilinmiyor"} / DDR{GetDdrType(obj["SMBIOSMemoryType"]?.ToString())}");
                sb.AppendLine($"     Part No     : {obj["PartNumber"]?.ToString()?.Trim()}");
        ...
        try
        {
            ...
            foreach (ManagementObject obj in searcher.Get())
            {
                ...
                string boyut = boyutBytes > 0
                    ? $"{boyutBytes / (1024UL * 1024 * 1024)} GB ({boyutBytes / (1024UL * 1024 * 1024 * 1024.0):F2} TB)"
                    : "Bilinmiyor";
                ...
                sb.AppendLine($"     Seri No     : {obj["SerialNumber"]?.ToString()?.Trim()}");
